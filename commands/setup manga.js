module.exports.run = async (client, message, args, functions) => {
    ...
    fs.readdir(require('path').join(`./servers/${message.guild.id}/`, "manga"), function (err, result) {
        ...
        .then(collected => {
            ...
            let data = JSON.parse(fs.readFileSync(`./servers/${message.guild.id}/manga/${message.content}.json`));
            data.lienChap = m.content.replace(/([0-9]+#?[0-9]*)$/g, "");

            fs.writeFile(`./servers/${message.guild.id}/manga/${message.content}.json`, JSON.stringify(data), (err) => {
